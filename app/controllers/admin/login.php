...
class Login extends \core\controller{
	...
	public function process(){
		...
		if (isset($_POST['submit']) && $_POST['submit'] == 'Secure Login') {
				...
				$password 		= isset($_POST['password']) ? $_POST['password'] : '';
					...
					$error[] = _ERROR_USERNAME_LANG_;
						...
						$user			= $this->_login->get_user($username);
							...
							if(Password::verify($password, $hash) === false) {
								...
								$postdata = array('IsLogin' => $yes, 'LastLogin' => $date, 'LastLoginIP' => $data['ip']);   
		...
		View::render('admin/dashboard/login',$data, $error);
	}
	...
	public function forgot(){
		...
		if(isset($_POST['submit'])) {
			...
			$result			= $this->_login->get_user_from_email($email);
		...
		View::render('admin/dashboard/forgot',$data, $error);
	}

	public function logout(){
		$no 		= 0;	
		$postdata 	= array('IsLogin' => $no);   
		$where 		= array('IdUser' => $_SESSION['cbn_IdUser']);   
