...
*
* @file       app/models/client/login.php
...
class Login extends \core\model {
	...
	public function get_hash($username) {
		$data =  $this->_db->select("SELECT Password FROM ".PREFIX."tclients WHERE Username = :username AND IsActive = 1", array(':username' => $username));
		return $data[0]->Password;
	}

	public function get_client($username) {
		return $this->_db->select("SELECT IdClient, FullName, Email, Username, ProfilePicture, LastLoginIP, IsLogin FROM ".PREFIX."tclients WHERE Username = :username AND IsActive = 1", array(':username' => $username));
	}

	public function get_client_from_email($email) {
		return $this->_db->select("SELECT IdClient, FullName, Email, Username FROM ".PREFIX."tclients WHERE Email = :email AND IsActive = 1 LIMIT 1", array(':email' => $email));
	}
